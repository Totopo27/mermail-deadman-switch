# Official Submission Package - Superteam Earn
...
5. **Solana Anchor Smart Contract (`mermail_deadman_vault`)**: PDA-governed vault (`[b"deadman_vault", owner]`, Program ID: `E4dA4YrWnMgFv7NNseHjw8r2yikArPGiEnrxX4YYExdX`) with SPL Token (USDC) escrow, Pyth Network real-time USD price feeds (`SOL/USD`), Squads Multisig compatibility, and legal oracle bypasses (`attest_oracle_trigger`).
...
- **[TIER 2: Urgent Notice (Day 37)]**: Multi-channel warning alerting of impending contingency procedures via email and Telegram.
...
- **Upgrade Tx Hash (SPL + Pyth)**: `L98mtYU5jnTTuYeFqZiy6HDAy2ipaQBRiuBtZJGdbFJonTDZRqJdPmufMSo5sZFR1ZMgKEWpBRdTRPin82Bt8Rg`
...
## 3. Pull Request Text for GitHub (`Nudgen-Marketing/mermail-skills`)

### PR Title:
`feat(skills): add mermail-deadman-switch — autonomous digital inheritance & multi-asset contingency protocol`
...
4. **Anchor Smart Contract Companion**: PDA-governed vault on Solana with SPL Token (USDC) escrow, Pyth Network price feeds, Squads multisig compatibility, and legal oracle bypass support (`attest_oracle_trigger`). Live deployed at `E4dA4YrWnMgFv7NNseHjw8r2yikArPGiEnrxX4YYExdX`.
